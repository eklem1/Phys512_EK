...
x=np.random.uniform(2.0, gridSize[0]-2.0,(n,3)) #need to place closer to center
...
for k in range(60):
    ...
    plt.savefig('out_noP_{}.jpg'.format(k), dpi=1200)
...
fig = plt.figure()
plt.plot(steps, save_E_w[:,2], label="Potential E") #hmm this is constant or very low

plt.title("Energy over time")
plt.xlabel("time step")
plt.ylabel("Energy")
plt.legend()
plt.savefig('Energy_noP_justPo.jpg')
